        ...
        if (parsed.isText) {
          ...
          const res = await fetch("/api/suppliers/search", {
            ...
            body: JSON.stringify({
              ...
              // Решение владельца (18.08): в ВЫДАЧЕ поиска — только точные
              // совпадения артикула (объединённый оригинал + двойники, 1-3
              // карточки). Заменители с другими номерами показываются ВНУТРИ
              // карточки товара («Аналоги в продаже» + каталог Laximo).
              // withAnalogs НЕ шлём — заодно поиск быстрее (без кроссов).
              ...(brand
                ? { brand }
                : parsed.brand
                ? { brand: parsed.brand }
