...
- Подтверждение email новых регистраций по умолчанию **ВКЛ** (как было на Supabase):
  регистрация шлёт письмо со ссылкой `/auth/confirm?token=…`, вход заблокирован до
  клика; по ссылке email подтверждается и пользователь сразу логинится. Отключить:
  `AUTH_REQUIRE_EMAIL_CONFIRM=false`. Перенесённые из Supabase уже подтверждены.
  Если ссылка протухла (24ч) — пользователь может задать пароль через «забыли пароль»
  (сброс тоже подтверждает email). «Повторно выслать подтверждение» — возможный TODO.
