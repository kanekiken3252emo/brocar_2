...
export const cartItems = pgTable("cart_items", {
  ...
  // Снимок НАШЕЙ цены оффера на момент добавления. Фиксируется в строке, чтобы:
  //  1) цена не «ехала» при перезаписи общего products.ourPrice (другой add того
  //     же артикула, ночной импорт);
  //  2) разные офферы одного артикула жили РАЗНЫМИ строками (по цене).
  // null = легаси-строка (создана до миграции) → цена берётся из products.ourPrice.
  price: numeric("price"),
