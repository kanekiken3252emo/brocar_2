  ...
  <div style="background:#f4f4f5;padding:24px 12px;font-family:Arial,sans-serif">
    <div style="max-width:640px;margin:0 auto;background:#ffffff;border:1px solid #eee;border-radius:12px;overflow:hidden">
      ...
      <div style="background:#f4f4f5;padding:20px 24px;color:#666;font-size:13px;text-align:center;border-top:1px solid #eee;line-height:1.7">
        ...
        Телефон: <a href="tel:+79326006052" style="color:#ea580c;text-decoration:none">+7 (932) 600-60-52</a> · <a href="tel:+73433822062" style="color:#ea580c;text-decoration:none">8 (343) 382-20-62</a><br>
...
/**
 * Письмо «Подтверждение email» — со ссылкой, по которой аккаунт активируется и
 * пользователь сможет войти. Используется при регистрации (локальная авторизация).
 * Бросает ошибку при сбое SMTP — роут /api/auth/register её логирует.
 */
export async function sendEmailConfirmation(
  ...
  const transporter = getTransporter();
  if (!transporter) {
    console.warn("SMTP не настроен — письмо подтверждения не отправлено");
    throw new Error("SMTP не настроен");
  }
  if (!isEmail(to)) throw new Error("Некорректный email");
  ...
  <div style="background:#f4f4f5;padding:24px 12px;font-family:Arial,sans-serif">
    <div style="max-width:560px;margin:0 auto;background:#ffffff;border:1px solid #eee;border-radius:12px;overflow:hidden">
      ...
      <div style="padding:28px 24px;color:#222">
        <h2 style="color:#ea580c;margin:0 0 12px">Подтвердите email</h2>
        <p style="margin:8px 0;line-height:1.6">
          Спасибо за регистрацию на ${esc(siteName)}! Остался один шаг — подтвердите,
          что <b>${esc(to)}</b> ваш. Нажмите кнопку, и можно будет войти.
          Ссылка действует 24 часа.
        </p>
        <div style="text-align:center;margin:28px 0">
          <a href="${confirmUrl}" style="display:inline-block;background:#ea580c;color:#fff;text-decoration:none;font-weight:bold;padding:14px 28px;border-radius:10px">
            Подтвердить email
          </a>
        </div>
        <p style="margin:8px 0;color:#666;font-size:13px;line-height:1.6">
          Если кнопка не работает, скопируйте ссылку в браузер:<br>
          <a href="${confirmUrl}" style="color:#ea580c;word-break:break-all">${esc(confirmUrl)}</a>
        </p>
        <p style="margin:16px 0 0;color:#888;font-size:13px;line-height:1.6">
          Если вы не регистрировались на ${esc(siteName)} — просто проигнорируйте это письмо.
        </p>
      </div>
      <div style="background:#f4f4f5;padding:18px 24px;color:#666;font-size:13px;text-align:center;border-top:1px solid #eee;line-height:1.7">
        <div style="font-weight:bold;color:#333;margin-bottom:6px">${esc(siteName)} — автозапчасти, г. Екатеринбург</div>
  ...
  await transporter.sendMail({
    from: `"${siteName}" <${from}>`,
    to,
    subject: `Подтвердите email — ${siteName}`,
    html,
  });
}
