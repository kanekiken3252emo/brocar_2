...
/**
 * Проверка ПЕРЕД живым платным вызовом (внутри compute кэша). Бросает ошибку,
 * если вызов делать нельзя — тогда ничего не кэшируется. Используется для
 * гостевого лимита (см. guest-limit.ts).
 */
export type LiveCallGuard = () => Promise<void>;
