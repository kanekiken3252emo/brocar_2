...
const wave2Products = Array.isArray(wave2Payload.products)
  ...
  : [];
const priorProducts = [...wave1Products, ...wave2Products];
...
if (selected.length !== TARGET_SIZE) {
  throw new Error(
    `Could not fill wave 3: selected ${selected.length} of ${TARGET_SIZE}`
  );
}
...
if (new Set(selectedKeyList).size !== selectedProducts.length) {
  throw new Error("Wave 3 contains duplicate product identities");
}
if (selectedKeyList.some((key) => priorKeys.has(key))) {
  throw new Error("Wave 3 overlaps a prior wave");
}
if (selectedProducts.some((row) => isVin(row.article))) {
  throw new Error("Wave 3 contains a VIN-like article");
}
...
const manifest = {
  ...
  selectionRule:
    "current stock and positive price; first remaining direct external article demand, then products with image, defined category and a brand represented in prior waves; category and brand caps; no overlap with waves 1-2; max 3 brands per article",
